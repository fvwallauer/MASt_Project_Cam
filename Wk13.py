...
# tiers 0,1,2 correspond to physical radii 290, 298, 306 mm
# -----------------------------------------------------------------
pivot_radii = jnp.asarray([290, 298, 306])

# fibre_span_allowed = jnp.asarray([r*0.4, r*0.7, r*1.3])  # for tiers 0,1,2 (low-high)
fibre_span_allowed = jnp.asarray([311, 233, 276]) # for tiers 0,1,2 (low-high), via Gavin Dalton


# -----------------------------------------------------------------
# tier-dependent maximum angular deviation from radial
# -----------------------------------------------------------------
phi_max_per_tier = jnp.deg2rad(jnp.asarray([9.0, 14.1, 14.1]))

# -----------------------------------------------------------------
# Build per-fibre pivot positions
# -----------------------------------------------------------------
fibre_ids = jnp.arange(n_fibres)
r_id_for_fibre = fibre_ids // n_tiers
t_id_for_fibre = fibre_ids % n_tiers

pivot_radius_for_fibre = pivot_radii[t_id_for_fibre]
angle_for_fibre = angles[r_id_for_fibre]

pivot_x = jnp.asarray(pivot_radius_for_fibre * jnp.cos(angle_for_fibre))
pivot_y = jnp.asarray(pivot_radius_for_fibre * jnp.sin(angle_for_fibre))

# For plotting: include the full pivot ring
plot_radius = float(np.max(np.array(pivot_radii))) * 1.08
...
def TF_reach_matrix(px, py):
    ...
    # -----------------------------------------------------------------
    # use per-fibre pivot positions rather than a single field-edge ring
    # -----------------------------------------------------------------
    rx = pivot_x                          # (F,)
    ry = pivot_y                          # (F,)
    tier_span = fibre_span_allowed[t_ids] # (F,)
    phi_allowed = phi_max_per_tier[t_ids] # (F,)
...
def all_button_vertices(px, py, allowed):
    ...
    # -----------------------------------------------------------------
    # use per-fibre pivot positions directly
    # -----------------------------------------------------------------
    rx = pivot_x[:, None]
    ry = pivot_y[:, None]
...
@jax.jit
def compute_path_bboxes(px, py, pivot_x, pivot_y, allowed):
    F, T = allowed.shape

    # -----------------------------------------------------------------
    # NEW: path boxes are built from per-fibre pivot positions
    # -----------------------------------------------------------------
    rx = pivot_x[:, None]
    ry = pivot_y[:, None]
...
@jax.jit
def compute_fibre_segments(px, py, pivot_x, pivot_y, allowed):
    F, T = allowed.shape
    target_pos = jnp.stack([px, py], axis=-1)
    start = jnp.broadcast_to(target_pos[None, :, :], (F, T, 2))

    # -----------------------------------------------------------------
    # NEW: fibre segment endpoint is the actual fibre pivot position
    # -----------------------------------------------------------------
    end = jnp.stack([pivot_x[:, None], pivot_y[:, None]], axis=-1)  # (F,1,2)
...
px, py = rand_targets(field_radius, n_targets)
...
fibre_start, fibre_dir = compute_fibre_segments(px, py, pivot_x, pivot_y, allowed)
...
# -----------------------------------------------------------------
# NEW: keep NumPy copies of the actual fibre pivot positions
# -----------------------------------------------------------------
pivot_x_np = np.array(pivot_x)
pivot_y_np = np.array(pivot_y)
t_id_np = np.array(t_id_for_fibre)  # fibre -> tier id
...
def plot_assignment(snapshot, filename, title="Assignment"):
    ...
    # Retractors
    # -----------------------------------------------------------------
    # NEW: draw the actual fibre pivot locations, not the old 205 mm ring
    # -----------------------------------------------------------------
    ax.scatter(
        pivot_x_np,
        pivot_y_np,
