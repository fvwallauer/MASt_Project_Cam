...
# Generate random target dist., uniform over circular sky area
...
# Make per-target padded geometry (always same dimension!!) from existing vertices, fibre_start, fibre_dir
...
for i in tqdm(range(0, len(pairs), B), desc="Building fibre-level collision blocks"):
    ...
    # Store pairs and blocks for this batch
    pairs_j_list.append(jnp.array(batch, dtype=jnp.int32))
    blocks_j_list.append(blocks_full)

# Concatenate all batches into final arrays for GPU storage
pairs_j = jnp.concatenate(pairs_j_list, axis=0)     # (Npairs, 2)
blocks_j = jnp.concatenate(blocks_j_list, axis=0)   # (Npairs, max_F, max_F)

print("pairs_j shape:", pairs_j.shape)
print("blocks_j shape:", blocks_j.shape)

# Create a lookup table for pair indices: (T, T) with -1 for non-pairs, and pair index for pairs
T = int(n_targets)  # number of targets
pair_index_np = -np.ones((T, T), dtype=np.int32)

# pairs is NumPy (Npairs,2) with t1<t2
for k, (t1, t2) in enumerate(pairs):
    pair_index_np[int(t1), int(t2)] = k
    pair_index_np[int(t2), int(t1)] = k  # symmetric lookup
...
"""
n_fibres, n_targets, n_tiers, max_F (previously defined)
allowed_np          (F, T)       bool
always_collide_mat  (T, T)       JAX bool  (we convert to numpy)
pair_index_np       (T, T)       int32     (-1 = no check needed)
blocks_j            (Npairs, max_F, max_F)  JAX bool
target_fibre_ids    (T, max_F)   int32     (-1 = padding)
target_fibre_mask   (T, max_F)   bool
px, py              (T,)         target positions
retractor_x, retractor_y         retractor positions
r_id_for_fibre      (F,)         retractor id per fibre
"""

always_collide_np = np.array(always_collide_mat)   # (T, T)
blocks_np = np.array(blocks_j)                     # (Npairs, max_F, max_F)
px_np = np.array(px)
py_np = np.array(py)
retractor_x_np = np.array(retractor_x)
retractor_y_np = np.array(retractor_y)
r_id_np = np.array(r_id_for_fibre)

# ======================================================================
# STATE ARRAYS
# ======================================================================
# fibre_to_target[f] = target assigned to fibre f, or -1
# target_to_slot[t]  = which padded fibre-slot is on target t, or -1
# target_to_fibre[t] = which global fibre id is on target t, or -1
...
# reverse mapping: for each fibre, which (target, slot) pairs is it compatible with?
fibre_to_possible = [[] for _ in range(n_fibres)]
for t in range(n_targets):
    for s in range(max_F):
        if target_fibre_mask[t, s]: # if True, this fibre slot is valid for target t
            f = int(target_fibre_ids[t, s]) 
            fibre_to_possible[f].append((t, s)) # for fibre f, add compatible (target, slot) pairs based on target_fibre_ids and mask

# ======================================================================
# Priorities
# ======================================================================

# Random priorites: 50/50 split of priority 10 and 1 (like WL mid-tier, see Hughes)
...
# Compute straightness for each fibre-target combination
# s = angular_deviation from inward norm / phi_max, phi_max = 14 deg
straightness = np.zeros((n_targets, max_F), dtype=np.float32)
phi_max = np.deg2rad(14.0)
for t in range(n_targets):
    for s in range(max_F):
        if not target_fibre_mask[t, s]: # if this fibre slot is not valid for target t, skip
            continue
        f = int(target_fibre_ids[t, s]) # global fibre ID for this target-slot pair
        rid = int(r_id_np[f]) # retractor ID for this fibre
        vx = px_np[t] - retractor_x_np[rid] # x-component of vector from retractor to target
        vy = py_np[t] - retractor_y_np[rid] # y-component of vector from retractor to target
        # radial (inward) direction is from retractor to target, so retractor vector is negative of that
        rx, ry = -retractor_x_np[rid], -retractor_y_np[rid]
        cos_a = (vx*rx + vy*ry) / (np.hypot(vx,vy)*np.hypot(rx,ry) + 1e-12) # as before
        straightness[t, s] = np.arccos(np.clip(cos_a, -1, 1)) / phi_max


# ======================================================================
# COLLISION LOOKUP to use during assignment checks
# ======================================================================

def collides(ti, si, tj, sj):
    """Check collision status of fibre in fibre-slot si on target ti and
       fibre in fibre-slot sj on target tj. 
       Returns True if they collide, False otherwise."""
    if always_collide_np[ti, tj]: # targets ti & tj always collide due to proximity
        return True
    k = pair_index_np[ti, tj] # lookup pair index for targets ti & tj; -1 if no fibre-level check needed (never collide)
    if k == -1:
        return False
    return bool(blocks_np[k, si, sj]) # if pair index k exists, check fibre-level collision block for fibre slots si & sj; return True if collide, False if not


def placement_valid(ti, si):
    """Check compatibility of fibre in slot si on target ti with ALL current assignments. 
    Returns True if compatible (no collisions), False if any collision."""
    for tj in range(n_targets): # for a given target
        if target_to_fibre[tj] == -1 or tj == ti: # if target tj is unassigned, or is the same target as ti, skip
            continue
        sj = target_to_slot[tj] # get assigned fibre slot on target tj
        if collides(ti, si, tj, sj):
            return False # if any collision found, return False immediately
    return True # if no collisions found with any assigned target, return True


def placement_valid_excluding(ti, si, exclude):
    """AS ABOVE but skip checking targets in 'exclude' set (used for swap checks).
        Returns True if compatible with all assigned targets except those in 'exclude'."""
...
def find_slot(fibre_id, target_id):
    """Find the padded slot index for a given fibre on a given target. -1 if unreachable."""
    for s in range(max_F): # iterate over padded fibre slots for target_id, up to max_F
        if target_fibre_mask[target_id, s] and target_fibre_ids[target_id, s] == fibre_id: # if this slot s on target_id is valid, and corresponds to fibre_id
            return s
    return -1
...
def unassign_fibre(f):
    t = fibre_to_target[f] # check which target is currently assigned to fibre f
    if t == -1: # if fibre f is currently unassigned, return -1, -1 to indicate no unassignment done
        return -1, -1
    s = target_to_slot[t] # get assigned fibre slot on target t
    fibre_to_target[f] = -1 # unassign fibre f
    target_to_slot[t] = -1 # unassign slot on target t
    target_to_fibre[t] = -1 # unassign fibre on target t
    return int(t), int(s) # return the target and slot that were unassigned from fibre f

def unassign_target(t): # unassign whatever fibre is currently assigned to target t, and return that fibre and slot
    f = target_to_fibre[t] # check which fibre is currently assigned to target t
    if f == -1: # if target t is currently unassigned, return -1, -1 to indicate no unassignment done
        return -1, -1
    s = target_to_slot[t]
    fibre_to_target[f] = -1
    target_to_slot[t] = -1
...
# =============================================================================
# ENERGY for minimization - Hughes (2022) eq. 4:  E_fibre = (1 + s) / priority
# =============================================================================

UNASSIGNED_PENALTY = 2   # >= worst-case (1+1)/1 = 2 in energy

def energy_single(t, s): # energy contribution of fibre assigned to target t in slot s, based on straightness and priority
    return (1.0 + straightness[t, s]) / priorities[t]

def energy_total(): # total energy of current assignment state, summing over all fibres; unassigned fibres contribute UNASSIGNED_PENALTY
    E = 0
    for f in range(n_fibres):
        ...
        if t == -1:
            ...
            E += energy_single(t, target_to_slot[t])
    return E

# =============================================================================
# GREEDY RANDOM INITIAL ASSIGNMENT
# =============================================================================

def init_assignment(seed=42):
    rng = np.random.default_rng(seed) # create new instance of rng
    order = rng.permutation(n_fibres) # random ordering of fibre ids from 0 to n_fibres-1
    placed = 0 # counter for how many fibres successfully placed
    for f in order: # iterate over fibres in random order
        candidate_placements = list(fibre_to_possible[f]) # get list of (target, slot) pairs that fibre f can potentially be assigned to
        rng.shuffle(candidate_placements) # shuffle candidate placements to add randomness to initial assignment (Terrett)
        for (t, s) in candidate_placements: # iterate over candidate placements for fibre f
            if target_to_fibre[t] != -1: # if target t is already assigned to some fibre, skip this candidate placement (no swapping in initial assignment)
                continue
            if placement_valid(t, s): # check if placing fibre f in slot s on target t would cause any collisions with already assigned fibres; if not valid, skip this candidate placement
                assign(f, t, s) # 
                placed += 1
                break
...
def accept(dE, T, rng):
    "Metropolis criterion, via Hughes (2022)"
    if dE <= 0:
        return True
    return rng.random() < np.exp(-1000*dE / T)

def anneal(T_i=10, T_f=0.1, delta_T=0.01, max_iters=20, seed=42):
    """
    Run simulated annealing on the current assignment state.

    Parameters (from Miszalski 2006):
        T_i       : initial temperature
        T_f       : final temperature
        delta_T   : cooling factor (T *= 1 - delta_T between each step)
        max_iters : fibre traversals per temperature step
        seed      : RNG seed
    """
    ...
    best_E = energy_total() # compute initial energy
    print(f"Initial state: E={best_E:.2f}, T={T:.4f}, "
          f"assigned={int(np.sum(fibre_to_target >= 0))}")
    while T > T_f:
        for _ in range(max_iters):
            # Traverse all fibres in random order
            for fibre_a in rng.permutation(n_fibres):
                possible = fibre_to_possible[fibre_a] # list of (target, slot) pairs that fibre_a can be assigned to
                if len(possible) == 0: # if fibre_a has no possible assignments, skip to next fibre
                    continue

                # Pick random reachable target (B)
                target_b, slot_ab = possible[rng.integers(len(possible))] # randomly select a (target, slot) pair from possible assignments for fibre_a
                target_a = fibre_to_target[fibre_a] # current assigment of fibre_a: -1 if free
                fibre_b = target_to_fibre[target_b] # another possible assigment of fibre_a: -1 if free
                ...
                # Determine swap type & check feasibility, via Miszalski (2006)
                if target_a == -1 and fibre_b == -1:
                    # Case 4: fibre_a is unassigned, target_b is unassigned -> assign fibre_a to target_b if valid
                    ...
                    # Case 2: fibre_a is assigned to target_a, target_b is unassigned -> move fibre_a from target_a to target_b if valid
                    slot_aa = target_to_slot[target_a] # current slot of fibre_a on target_a
                    if placement_valid_excluding(target_b, slot_ab, exclude={target_a}): 
                        # check if placing fibre_a in slot_ab on target_b would cause any 
                        # collisions with currently assigned fibres, excluding target_a 
                        # (since we're moving fibre_a away from target_a, we don't need to 
                        # check for collisions with target_a's current assignment)
                        dE = energy_single(target_b, slot_ab) - energy_single(target_a, slot_aa)
                        if accept(dE, T, rng):
                            unassign_fibre(fibre_a) # unassign fibre_a from target_a
                            assign(fibre_a, target_b, slot_ab)
                            accepted = True
                            swap_counts[2] += 1
                
                elif target_a == -1 and fibre_b != -1:
                    # Case 3: fibre_a is unassigned, target_b is assigned to fibre_b -> move fibre_a to target_b, unassign fibre_b
                    slot_bb = target_to_slot[target_b]
                    if placement_valid_excluding(target_b, slot_ab, exclude={target_b}):
                        # check if placing fibre_a in slot_ab on target_b would cause any collisions with currently assigned fibres, excluding target_b
                        dE = energy_single(target_b, slot_ab) - energy_single(target_b, slot_bb) # E_new - E_old
                        if accept(dE, T, rng):
                    ...
                    # Case 1: fibre_a on target_a, fibre_b on target_b -> fibre_a on target_b, fibre_b on target_a (swap) IF VALID
                    slot_aa = target_to_slot[target_a]
                    slot_bb = target_to_slot[target_b]
                    slot_ba = find_slot(fibre_b, target_a) # find slot index for fibre_b on target_a; -1 if fibre_b cannot be assigned to target_a
                    if slot_ba == -1:
                        continue

                    check1 = placement_valid_excluding(target_b, slot_ab, exclude={target_a, target_b}) # check if placing fibre_a in slot_ab on target_b would cause any collisions with current field, excluding target_a and target_b. Returns True if valid, False if any collision.
                    ...
                    if check1 and check2 and check3:
                        dE = (energy_single(target_b, slot_ab) + energy_single(target_a, slot_ba)) - (energy_single(target_a, slot_aa) + energy_single(target_b, slot_bb))
                        if accept(dE, T, rng):
                            # perform swap
        ...
        T *= (1 - delta_T) # cool down temperature

        # Log progress
        current_E = energy_total()
        n_assigned = np.sum(fibre_to_target >= 0)
        n_pri10 = np.sum((fibre_to_target >= 0) &
                             np.array([priorities[t] == 10 if t >= 0 else False
                                       for t in fibre_to_target]))
        print(f"  T={T:.4f}  E={current_E:.2f}  assigned={n_assigned}  "
              f"pri10={n_pri10}  accepted={n_accepted}  steps={n_steps}")

    print(f"\nAnnealing complete. Swap counts: {swap_counts}")
    print(f"Final: E={energy_total():.2f}, "
          f"assigned={int(np.sum(fibre_to_target >= 0))}/{n_fibres}")


init_assignment(seed=42)
print(f"Initial energy: {energy_total():.2f}\n")

anneal(T_i=10.0, T_f=0.1, delta_T=0.01, max_iters=20, seed=123)
